...
- Vercel project creation, environment variables, and deployment;
...
| Vercel                | Public HTTPS hosting for frontend/API, OAuth, and webhooks       | Code prepared; deployment pending |
...
Before the first deployment, add these environment variables for **Production** and **Preview**:

| Variable               | Value source                                      |
| ---------------------- | ------------------------------------------------- |
| `NODE_ENV`             | `production`                                      |
| `DATABASE_URL`         | Supabase transaction pooler URL on port `6543`    |
| `SESSION_COOKIE_NAME`  | The value already used in `.env.local`            |
| `SESSION_TTL_SECONDS`  | The value already used in `.env.local`            |
| `TOKEN_ENCRYPTION_KEY` | The existing server-only Base64 encryption secret |
